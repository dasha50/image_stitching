...
%所有可以用来匹配的特征点
inliers=inliers(1:4:end);%使特征点分散，不要太密集
...
flag=zeros(length(inliers),1); %初始化所有特征点的seedcount值，表示有没有被选为seed点，0为没有被选过
sum_threshold=0.01; %阈值
...
%penalty(seednum)=penalty(seednum)+1;    %被选择的特征点对应的penalty值+1
...
for i=6:28
    ...
    %penalty(index(i))=penalty(index(i))+1;
    %--------
    %--求Hg--
    %--------
    ...
    %求经过Hs变换的四角坐标
    C1s=Hs*[1;1;1];   %左上
    C2s=Hs*[size(img2,2);1;1];    %右上
    C3s=Hs*[1;size(img2,1);1];    %左下
    C4s=Hs*[size(img2,2);size(img2,1);1];     %右下   
    sum=norm(C1bar-C1s)+norm(C2bar-C2s)+norm(C3bar-C3s)+norm(C4bar-C4s);
    %距离之和归一化
    img2_size=size(img2,1)*size(img2,2);
    norm_sum=sum/img2_size;
    norm_sum
    %判断H是否满足预先设置的阈值
...
%{
if length(featrues_group_idx)==length(index)%如果特征点组选了所有特征点，则放弃
    continue;
end
%}
...
%计算差值图Ed
%canny检测边缘，得到边缘图
gray_warped_img1=rgb2gray(warped_img1_ds);
gray_warped_img2=rgb2gray(warped_img2_ds);
