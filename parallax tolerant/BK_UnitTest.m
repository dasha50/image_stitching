...
%    platform and then exercises the library to look for silly bugs.
    ...
    function D = Sparse2Dense(S)    
        [i,j,s] = find(S);  %找S中的n个非零元素，s为n维向量[1;1;1...1;1],
        z = zeros(size(s,1),1); %z为n维向量[0;0;0...0;0]
...
BK_SetPairwise(h, Sparse2Dense([0 2 0 0 0;
                                ...
                                0 0 0 0 5;
                                0 0 0 0 0]));
                            %用到了Sparse2Dense函数，把各节点site间的weighs矩阵转换为edges矩阵
...
%twister是随机数产生器,这句话定义伪随机数的起点，以产生重复的伪随机数
wd = 128; ht = 96;
noise1 = rand([wd,ht])*20;
noise2 = rand([wd,ht])*20;
H = fspecial('disk',5);%定义2D滤波器，为圆形区域均值滤波，区域半径为5
...
for y=1:ht 
    %连接edge，连接像素点与邻域内4点
    for x=1:wd
        %把(x,y)与分别与(x+1,y)和(x,y+1)连接起来，edge cost为1
        if (x < wd), nb((y-1)*wd+x,(y-1)*wd+x+1) = 1; end
        if (y < ht), nb((y-1)*wd+x, y   *wd+x  ) = 1; end
    end
end
distmap = [zeros(ht,wd/2) ones(ht,wd/2)];%distmap左半边为全0，右半边为全1
distmap = bwdist(distmap);%计算距离图
distmap = distmap / max(distmap(:));%归一化
distmap1 = distmap;
distmap2 = flipdim(distmap,2);%将distmap左右翻转
distmap = [distmap1(:)'; distmap2(:)'];

hinc = BK_Create(wd*ht,2*wd*ht);%总共有wd*ht个节点，2*wd*ht条边
